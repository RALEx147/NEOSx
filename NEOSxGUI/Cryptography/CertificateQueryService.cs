...
namespace Neo.Cryptography
{
    internal static class CertificateQueryService
    {
        ...
        {
            Directory.CreateDirectory(Properties.Settings.Default.Paths.CertCache);
        }
        ...
        private static void Web_DownloadDataCompleted(object sender, DownloadDataCompletedEventArgs e)
        {
            using ((WebClient)sender)
            {
                ...
                else
                {
                    ...
                    string path = Path.Combine(Properties.Settings.Default.Paths.CertCache, $"{address}.cer");
        ...
        private static void UpdateResultFromFile(UInt160 hash)
        {
            ...
            try
            {
                cert = new X509Certificate2(Path.Combine(Properties.Settings.Default.Paths.CertCache, $"{address}.cer"));
            }
            ...
            if (!hash.Equals(Contract.CreateSignatureRedeemScript(ECPoint.DecodePoint(cert.PublicKey.EncodedKeyValue.RawData, ECC.ECCurve.Secp256r1)).ToScriptHash()))
            {
                results[hash].Type = CertificateQueryResultType.Missing;
                return;
            }
